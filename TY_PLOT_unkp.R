...
names.fi1 = c("OTP","MOL","RIC","MTEL","OPUS",  "HID","OMV","TGN","COTE","SNG",   "SNN","TEL","MON","CEZ","KOM",  "PKO","CDR","PKN","PZU")
names.fi2 = c("A1","A2","A3","A4","A5","K1","K2","K3","K4","K5","M1","M2","M3","M4","M5","C1","C2","C3","C4","C5")
...
groups1   = list(1:5,6:10,11:15,16:20)
# 


col1      = c(rep("red", ((ncol(con1))/4)), 
             rep("blue", ((ncol(con1))/4)), 
             rep("green4", ((ncol(con1))/4)),
             rep("purple",((ncol(con1))/4)))

nodecol1=c(rep("white", (ncol(con1))))
# 
...
repu=0.9
 ...
 nodecol=nodecol1
 node_col=col
 legendstring=c("Magyarország","Románia","Csehország","Lengyelország")
#legendstring1<-c("Level", "Slope", "Curvature")
legendstring2<-c("red", "blue","green4", "purple")
...
toplot_l1<-con1[1:5,1:5]
toplot_l2<-con1[6:10,6:10]
toplot_l3<-con1[11:15,11:15]
...
if (plot_matrix["all", "all"]==1){
  ...
  con[slope_tags,curvature_tags]<-0
  groups=groups1
  col= c(rep("red", ((ncol(con1))/3)), 
         rep("blue", ((ncol(con1))/3)), 
         rep("green4", ((ncol(con1))/3)))
  nodecol=nodecol1
  node_col=col
...
if (plot_matrix["node", "node"]==1){
  ...
  col_t      = c(rep("#FF000033", ((ncol(con1))/3)), 
                 rep("#0000FF33", ((ncol(con1))/3)), 
                 rep("#008B0033", ((ncol(con1))/3)))
